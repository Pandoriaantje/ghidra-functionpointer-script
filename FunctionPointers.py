...
def backtrack_call(instruction, register, depth=0):
    ...
    if depth > MAXDEPTH:  # limiting recursive backtracking depth by setting MAXDEPTH
        return 0
        ...
        if instruction.flowType.call:  # For now we are only interested in CALLs 
            if isinstance(instruction.getOpObjects(0)[0], Register):
                ...
                if instruction.flowType.computed and instruction.referencesFrom:  # computed FlowType is an indirect call
                    ...
                    print('--> 0x{} {}'.format(instruction.address, instruction))
                    if func_ref:
                        print('\t\tIndirect calling function: {} {}'.format(func_ref.entryPoint, func_ref.name))
                    else:
                        print("\t\t[!] No function found at computed call address. Possible missing symbol.")
                else:
